...
public class CartServiceImpl implements CartService {
    ...
    public CartResponse addItem(AddToCartRequest request) {
        ...
        ProductVariant variant = productVariantRepository.findByIdAndActiveTrue(request.variantId())
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Variant not found or inactive: " + request.variantId()
                ));

        // ✅ INDUSTRY-CRITICAL GUARDS

        if (variant.getPrice() == null || variant.getPrice().signum() <= 0) {
            throw new BadRequestException(
                    "Variant has invalid price configuration: " + variant.getLabel()
            );
        }

        if (variant.getStock() == null || variant.getStock() <= 0) {
            throw new BadRequestException(
                    "Variant is out of stock: " + variant.getLabel()
            );
        }

        // ✅ stock validation (quantity safety)
    ...
    public CartResponse updateItem(Long cartItemId, UpdateCartItemRequest request) {
        ...
        // ✅ Protect against admin disabling variant after cart addition
